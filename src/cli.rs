...
use colored::*;
use crate::{BlocklistManager, BlocklistDownloader, Config, DnsServer, UpdateScheduler, Result};
...
"#;
    ...
    println!("  {}", "A fast, blocklist-driven DNS sinkhole".bright_black());
    println!();
}
    ...
    pub async fn execute(&self) -> Result<()> {
        match &self.command {
                ...
                match find_server_pid()? {
                    Some(pid) => {
                        ...
                        match find_server_pid()? {
                            ...
                            None => {
                                println!("  {} Server stopped successfully", "[ok]".bright_green().bold());
                            }
                        }
                    }
                    None => {
            ...
            Some(Commands::Reload { config: config_path }) => {
                ...
                match find_server_pid()? {
                    Some(pid) => {
                        ...
                        println!("  {} Reload signal sent successfully", "[ok]".bright_green().bold());
                        println!("  {} Blocklists are being reloaded with zero downtime", "[ok]".bright_green());
                    }
                    None => {
                        println!("  {} No running server found", "[x]".bright_red().bold());
                        println!("  {} Start the server first with: {} skypier-blackhole start",
                                "[i]".bright_blue(), "->".bright_white());
                    }
                }
            ...
            Some(Commands::Add { domain, config: config_path }) => {
                ...
                println!("  {} Domain added to: {}", "[ok]".bright_green(), config.blocklist.custom_list.bright_blue());
                ...
                match find_server_pid()? {
                    Some(pid) => {
                        ...
                        send_signal(pid, SIGHUP)?;
                        std::thread::sleep(std::time::Duration::from_millis(300));
                        println!("  {} Server reloaded, domain is now blocked", "[ok]".bright_green().bold());
                    }
                    None => {
                        println!("  {} Server not running - changes will apply on next start", "[i]".bright_yellow());
                    }
                }
            ...
            Some(Commands::Remove { domain, config: config_path }) => {
                ...
                if original_count == new_count {
                    ...
                    println!("  {} Domain removed from: {}", "[ok]".bright_green(), config.blocklist.custom_list.bright_blue());
                    ...
                    match find_server_pid()? {
                        Some(pid) => {
                            ...
                            send_signal(pid, SIGHUP)?;
                            std::thread::sleep(std::time::Duration::from_millis(300));
                            println!("  {} Server reloaded, domain is now allowed", "[ok]".bright_green().bold());
                        }
                        None => {
                            println!("  {} Server not running - changes will apply on next start", "[i]".bright_yellow());
                        }
            ...
            Some(Commands::Update { config: config_path }) => {
                ...
                if config.blocklist.remote_lists.is_empty() {
                    ...
                    println!();
                    println!("  {} Add remote sources to your config:", "[i]".bright_yellow());
                ...
                match downloader.download_multiple(&config.blocklist.remote_lists).await {
                    Ok(domains) => {
                        println!("  {} Downloaded {} unique domains", "[ok]".bright_green(), domains.len().to_string().bright_yellow().bold());
                        ...
                        match find_server_pid()? {
                            Some(pid) => {
                                ...
                                send_signal(pid, SIGHUP)?;
                                std::thread::sleep(std::time::Duration::from_millis(500));
                                println!("  {} Server reloaded successfully", "[ok]".bright_green().bold());
                    ...
                    Err(e) => {
                        println!("  {} Failed to download blocklists: {}", "[x]".bright_red(), e);
                        println!("  {} Check your internet connection and URLs", "[i]".bright_yellow());
                    }
                }
            ...
            Some(Commands::Test { domain, config: config_path }) => {
                ...
                if is_blocked {
                    println!("  {} Status: {}", "[x]".bright_red(), "BLOCKED".bright_red().bold());
                    println!("  {} This domain will be blocked by the DNS server", "[i]".bright_blue());
                    println!("  {} DNS queries will receive: {}", "->".bright_white(), "REFUSED".bright_yellow());
                } else {
                    println!("  {} Status: {}", "[ok]".bright_green(), "ALLOWED".bright_green().bold());
                    println!("  {} This domain will be resolved normally", "[i]".bright_blue());
